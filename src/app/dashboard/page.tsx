  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Hospital Nuestra Señora del Carmen</h1>
          <p className="text-sm text-zinc-400">Sistema Administrativo Interno</p>
        </div>
        <DashboardSelector />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="flex flex-col rounded-2xl border border-white/10 bg-zinc-900/50 p-6 shadow-xl">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-white">Finalización de vacaciones</h2>
              <p className="text-xs text-zinc-500">Próximos {daysRange} días</p>
            </div>
          ...
          <div className="flex flex-col gap-4">
            {vacacionesActivas && vacacionesActivas.length > 0 ? (
                ...
                return (
                  <div key={v.id} className="flex items-center justify-between rounded-xl bg-black/20 p-4">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">Hasta {formatDate(v.fecha_fin)}</p>
                    </div>
                    <div className="rounded-lg bg-orange-500/10 px-3 py-1.5 text-sm font-medium text-orange-500">
              ...
              <p className="text-sm text-zinc-500 text-center py-8">No hay finalizaciones en los próximos {daysRange} días.</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col rounded-2xl border border-white/10 bg-zinc-900/50 p-6 shadow-xl">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-white">Inicio de vacaciones</h2>
              <p className="text-xs text-zinc-500">Próximos {daysRange} días</p>
            </div>
          ...
          <div className="flex flex-col gap-4">
            {vacacionesProximas && vacacionesProximas.length > 0 ? (
              vacacionesProximas.map((v) => {
                ...
                return (
                  <div key={v.id} className="flex items-center justify-between rounded-xl bg-black/20 p-4">
                    <div>
                      <p className="font-medium text-zinc-200">{v.empleados.nombres} {v.empleados.apellidos}</p>
                      <p className="text-xs text-zinc-500">Desde {formatDate(v.fecha_inicio)}</p>
                    </div>
                    <div className="rounded-lg bg-blue-500/10 px-3 py-1.5 text-sm font-medium text-blue-400">
                      Inicia en {days} días
                    </div>
                  </div>
                )
              ...
              <p className="text-sm text-zinc-500 text-center py-8">No hay inicios programados en los próximos {daysRange} días.</p>
            )}
          </div>
        </div>
      </div>
