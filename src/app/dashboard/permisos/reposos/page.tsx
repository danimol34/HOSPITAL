...
export default function RepososPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/FORMATO DE PERMISOS - REPOSOS.docx')
      ...
      // Descargar archivo
      saveAs(out, `Reposo_${form.cedula}_${form.fecha_solicitud}.docx`)
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
      <div className="flex items-center justify-between rounded-2xl border border-sys-border bg-sys-panel/50 p-6 shadow-xl">
        <h1 className="text-2xl font-bold text-sys-text">Generar Permiso de Reposo</h1>
      </div>
      ...
      <div className="rounded-2xl border border-sys-border bg-sys-bg p-8 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-sys-text-muted uppercase">Fecha de Inicio (Desde)</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-sys-text-muted uppercase">Fecha de Culminación (Hasta)</label>
